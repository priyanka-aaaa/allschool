...
$sel = mysqli_query($conn, "SELECT  count(`receipt`.`id`) AS allcount from receipt INNER JOIN admission ON receipt.student_id=admission.id
INNER JOIN class ON admission.class_id=class.id where class.id='$class' and admission.school_id = '$school_id' and admission.session = '$session'" . $searchQuery);
$records = mysqli_fetch_assoc($sel);
$totalRecordwithFilter = $records['allcount'];
$sql5 = "SELECT receipt.id, receipt.date, admission.name,admission.name,section.section as sectionname ,receipt.total_paying_amount,receipt.balance_amount,receipt.remark,class.class_name FROM receipt
...
where class.id='$class' and receipt.date='$date' and admission.school_id = '$school_id' and admission.session = '$session'".$searchQuery . " ORDER BY " . $columnName . " desc  limit ".$row.",".$rowperpage;
