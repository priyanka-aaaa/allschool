...
INNER JOIN class ON admission.class_id=class.id where class.id='$class' and admission.section_id='$section' and admission.school_id = '$school_id' and admission.session = '$session'");
$records = mysqli_fetch_assoc($sel);
$totalRecords = $records['allcount'];
$sel = mysqli_query($conn, "SELECT  count(`receipt`.`id`) AS allcount from receipt INNER JOIN admission ON receipt.student_id=admission.id
INNER JOIN class ON admission.class_id=class.id where class.id='$class' and admission.section_id='$section' and admission.school_id = '$school_id' and admission.session = '$session'" . $searchQuery);
$records = mysqli_fetch_assoc($sel);
...
where class.id='$class' and admission.section_id='$section' and receipt.month='$month' and admission.school_id = '$school_id' and admission.session = '$session'".$searchQuery . " ORDER BY " . $columnName . " desc  limit ".$row.",".$rowperpage;
